...
interface MobileQRScannerProps {
  ...
  onConnection?: (userId: string) => void
}

export function MobileQRScanner({ open, onOpenChange, onConnection }: MobileQRScannerProps) {
      ...
      try {
        ...
        const scanQRCode = () => {
          ...
          if (context && videoRef.current.readyState === videoRef.current.HAVE_ENOUGH_DATA) {
            ...
            //
            // if (code) {
            //   // Check if the QR code is a valid GrowthLab profile URL
            //   try {
            //     const url = new URL(code.data)
            //     if (url.origin === process.env.NEXT_PUBLIC_APP_URL && url.pathname.startsWith('/connect/')) {
            //       const userId = url.pathname.split('/').pop()
            //       if (scanning) {
            //         setScanning(false)
            //         setScanResult({ userId, success: true })
            //       }
            //       return
            //     }
            //   } catch (e) {
  ...
  const handleCopyQRCode = () => {
    if (qrCodeData) {
      // In a real app, we would copy a shareable link instead of the data URL
      navigator.clipboard
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (onConnection) {
          onConnection(scanResult.userId)
        } else {
  ...
  // In the scanSuccessCallback function, add location data collection
  const scanSuccessCallback = async (decodedText: string) => {
    setScanning(false)
    ...
    try {
      ...
      try {
        if (navigator.geolocation) {
          ...
          locationData = {
            ...
            city: "Singapore", // Mock data
            country: "Singapore", // Mock data
          }
        }
      } catch (error) {
      ...
      // Store the location data for analytics purposes
      // You would typically send this to the server along with the scan data
      console.log("QR code scanned with location:", locationData)

      // Process the QR code data
      // const data = await processQRCode(decodedText);
      // if (data && onScan) {
      //   onScan(data);
      // }
    } catch (error) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[90vh] rounded-t-xl">
        <SheetHeader>
          <SheetTitle>QR Code Connection</SheetTitle>
          <SheetDescription>Connect with other users by sharing your QR code or scanning theirs.</SheetDescription>
        </SheetHeader>
            ...
            {qrCodeData ? (
              <>
                <div className="border-4 border-primary/10 rounded-lg p-2 bg-white">
                  <QRCodeCanvas value={qrCodeData} size={250} level="H" />
                </div>
