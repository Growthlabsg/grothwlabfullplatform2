...
export function EventsProvider({ children }: { children: ReactNode }) {
  ...
  // Load events and saved events from localStorage on mount
  useEffect(() => {
    ...
    const eventsData = localStorage.getItem('platformEvents')
    if (eventsData) {
  ...
  const unsaveEvent = (eventId: number) => {
    const newSaved = savedEvents.filter(id => id !== eventId)
    setSavedEvents(newSaved)
    localStorage.setItem('savedEvents', JSON.stringify(newSaved))
  }
