...
export default function FeedPage() {
  ...
  const handleCreatePost = () => {
    // Find the create post button in the FeedContent component
    const createPostButton = document.querySelector('[data-testid="create-post-trigger"]') as HTMLButtonElement
    if (createPostButton) {
      createPostButton.click()
    } else {
      // Fallback: try to find any button with "Start a post" text
      const startPostButton = Array.from(document.querySelectorAll('button')).find(
        button => button.textContent?.includes('Start a post')
          ...
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-xl font-bold text-gray-900 truncate">GrowthLab</h1>
